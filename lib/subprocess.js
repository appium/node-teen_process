...
const accessAsync = B.promisify(fs.access);
  ...
  async start (startDetector = null, timeoutMs = null, detach = false) {
    ...
    return await new B((resolve, reject) => {
      ...
      this.proc.on('error', async (err) => {
        ...
        if (err.errno === 'ENOENT') {
          try {
            await which(this.cmd);
            if (this.opts?.cwd && !await accessAsync(this.opts.cwd, fs.R_OK)) {
              err = new Error(`The current working directory '${this.opts.cwd}' for '${this.cmd}' command ` +
