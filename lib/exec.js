...
async function exec (cmd, args = [], opts = {}) {
  ...
  return await new B((resolve, reject) => {
    ...
    const handleStream = (streamType, streamProps) => {
      ...
      // keep track of the stream if we have not said not to
      ...
      const {buffer, maxSize} = streamProps;
      proc[streamType].on('data', (data) => {
        buffer.push(data);
        streamProps.size += _.size(data);
        while (buffer.length > 1 && streamProps.size >= maxSize) {
          streamProps.size -= _.size(_.first(buffer));
          buffer.shift();
        }
