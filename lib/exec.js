...
async function exec (cmd, args = [], opts = {}) {
  ...
  opts = Object.assign({
    ...
    shell: undefined,
    logger: undefined,
    stdoutBufferSize: DEFAULT_BUFFER_SIZE,
    stderrBufferSize: DEFAULT_BUFFER_SIZE,
  }, opts);
    ...
    if (proc.stdout) {
      ...
      if (opts.ignoreOutput) {
        ...
        proc.stdout.on('data', (data) => {
          if (stdoutArr.length >= opts.stdoutBufferSize) {
            stdoutArr.shift();
          }
          stdoutArr.push(data);
    ...
    if (proc.stderr) {
      ...
      if (opts.ignoreOutput) {
        ...
        proc.stderr.on('data', (data) => {
          if (stderrArr.length >= opts.stderrBufferSize) {
            stderrArr.shift();
          }
          stderrArr.push(data);
