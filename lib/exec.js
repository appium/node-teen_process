    ...
    if (proc.stdout) {
      ...
      if (opts.ignoreOutput) {
        ...
        proc.stdout.on('data', (data) => {
          stdoutArr.push(data);
          stdoutBufferSize += _.size(data);
          while (stdoutArr.length > 1 && stdoutBufferSize >= opts.maxStdoutBufferSize) {
            stdoutBufferSize -= _.size(_.first(stdoutArr));
            stdoutArr.shift();
          }
    ...
    if (proc.stderr) {
      ...
      if (opts.ignoreOutput) {
        ...
        proc.stderr.on('data', (data) => {
          stderrArr.push(data);
          stderrBufferSize += _.size(data);
          while (stderrArr.length > 1 && stderrBufferSize >= opts.maxStderrBufferSize) {
            stderrBufferSize -= _.size(_.first(stderrArr));
            stderrArr.shift();
          }
