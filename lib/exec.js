...
async function exec (cmd, args = [], originalOpts = /** @type {T} */({})) {
  ...
  return await new B((resolve, reject) => {
    ...
    const handleStream = (/** @type {string} */ streamType, /** @type {CircularBuffer} */ buffer) => {
      ...
      proc[streamType].on('data', (/** @type {Buffer} */ chunk) => {
        ...
        if (opts.logger && _.isFunction(opts.logger.debug)) {
          opts.logger.debug(iconv.decode(chunk, opts.encoding || 'utf8'));
        }
      });
    };
    ...
    function getStdio (isBuffer) {
      const stdout = isBuffer ? stdoutBuffer.value() : iconv.decode(stdoutBuffer.value(), opts.encoding || 'utf8');
      const stderr = isBuffer ? stderrBuffer.value() : iconv.decode(stderrBuffer.value(), opts.encoding || 'utf8');
